...
video_path_out = "video_output.mp4"
...
out = cv2.VideoWriter(video_path_out, cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, frame_height))
...
while cap.isOpened():
        ...
        for box in res.boxes:
            ...
            if label in target_classes and confidence >= objectsData.get(label,{}).get('minimum_confidence', confidence_min):
                ...
                if(objectsData.get(label,{}).get('display', True)):
                    ...
                    cv2.rectangle(frame, (x1, y1), (x2, y2), (objectsData.get(label,{}).get('B', 255),objectsData.get(label,{}).get('G', 255),objectsData.get(label,{}).get('R', 255))
                                , objectsData.get(label,{}).get('line_thickness', 1), lineType= line_type)
                    cv2.putText(frame, f"{objectsData.get(label,{}).get('display_name', label)} - {100*confidence:.2f}%", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, 
    ...
    if messageData.get(msg, {}):
        ...
        cv2.rectangle(frame, (5, 5), (text_width + 10, text_height + 10), 
        ...
        cv2.putText(frame, messageData.get(msg, {}).get('message', ' '), (10, 10+text_height), font_type, messageData.get(msg, {}).get('scale', 2), 
